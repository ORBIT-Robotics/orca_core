#!/usr/bin/env python3
"""Preflight every enabled ORCA hand serial bus."""
...
try:
    ...
    from orca_core.scripts._bus_preflight import preflight_motor_roles
    from orca_core.scripts._dynamixel_preflight import _role_motor_ids, _role_motor_type
...
def _print_plan(specs) -> None:
    print("ORCA hand bus preflight plan:")
    for spec in specs:
        ...
        if motor_type == "dynamixel":
            ...
            action = "read-only reachability check for every configured motor ID"
        else:
            action = "unsupported motor_type; fail before connecting runtime"
        print(
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Check all enabled ORCA hand serial buses before hardware startup. "
            "Dynamixel roles keep their status/reboot recovery; Feetech roles "
            "must respond to read-only status packets before the runtime connects."
        )
    )
    parser.add_argument(
        "--robot",
    ...
    return 0 if preflight_motor_roles(specs) else 1
