...
class FeetechClient(MotorClient):
    ...
    def disconnect(self, force: bool = False) -> None:
        ...
        try:
            ...
            if not force:
                raise
            logging.exception('Failed to disable Feetech torque during forced disconnect.')
    ...
    def _read_pos_vel_cur_individual(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        for i, motor_id in enumerate(self.motor_ids):
            ...
            if result == COMM_SUCCESS and error == 0:
                ...
                positions[i] = self._position_raw_to_rad(pos_normalized)
            else:
                logging.warning(
    ...
    def write_desired_current(
        ...
        for current in currents:
            torque_raw = self._torque_from_current(float(current))
    ...
    def _position_raw_to_rad(self, pos_raw: int) -> float:
        """Convert a Feetech raw position into ORCA runtime radians."""
        return pos_raw * self.pos_scale * POSITION_DIRECTION

    def _position_rad_to_raw(self, pos_rad: float) -> int:
        """Convert ORCA runtime radians into a Feetech raw position."""
        return int(pos_rad * POSITION_DIRECTION / self.pos_scale)

    def _torque_from_current(self, current: float) -> int:
        """Map ORCA's current-limit value to Feetech's torque-limit register."""
        return int(np.clip(abs(current), 0, 1000))
    ...
    def calibrate_offset(self, motor_id: int, upper: bool = True) -> bool:
        ...
        target_position = self._offset_calibration_target(upper)
